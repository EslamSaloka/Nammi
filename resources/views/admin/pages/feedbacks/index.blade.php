                    ...
                    <table class="table card-table table-striped table-vcenter text-nowrap mb-0">
                        ...
                        <tbody>
                            @foreach ($lists as $list)
                                <tr>
                                    <td>
                                        @canAny('feedbacks.edit')
                                        <a href="{{ route('admin.feedbacks.edit', $list->id) }}">
                                            {{ $list->name ?? '' }}
                                        </a>
                                        @else
                                            {{ $list->name ?? '' }}
                                        ...
                                        <div class="hstack gap-2 fs-15">
                                            @canAny('feedbacks.edit')
                                                @include('admin.component.buttons.edit_actions', [
                                                    'url' => route('admin.feedbacks.edit',$list->id),
                                                ])
                                            @endcanAny
                                            @canAny('feedbacks.destroy')
                                                @include('admin.component.buttons.delete_actions', [
                                                    'url' => route('admin.feedbacks.destroy',$list->id),
                                                ])
                                            @endcanAny
                                        </div>
